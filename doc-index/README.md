...
2. **STRUCTURE** — Markdown по ATX-заголовкам **`#` … `######`** (уровни 1–6); преамбула до первого заголовка — `(preamble)`; для `.kt` — логически по файлу. Тело секции сначала набирается до лимита с учётом резерва под первую строку чанка (`STRUCTURE_HEADING_LINE_RESERVE` в `ModelConstants`); если не помещается — склейка **абзацев** (разделитель двойной перевод строки), затем при необходимости окна **1200 / 200** по символам. В начале поля `text` у STRUCTURE-чанка дублируется **название раздела** (строка + пустая строка), чтобы эмбеддинг учитывал заголовок, которого нет в теле под `##` в исходнике. В `index_meta` поле `schema_version`: при смене формата чанков нужно пересобрать индекс (**`2`** — текущая версия после доработок STRUCTURE).

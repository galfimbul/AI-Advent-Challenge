...
Поток данных **от UI до вызова OpenAI API** для этого экрана описан в пункте **Поток** ниже (цепочка **AgentScreen** → **AgentViewModel** → **Agent.process** → **ChatRepository.sendMessage** → **OpenAiApi**).
...
- **UI:** отображение текущей стратегии; кнопка «Настройки» (иконка) рядом с полем ввода; настройки — семь сворачиваемых блоков (аккордеон), в т.ч. «Локальный индекс (RAG)» с переключателем и блок «Инварианты» с полем ввода и «Сохранить»; при Facts — блок «Факты из диалога»; при Ветки — вкладки и «Создать ветку»; чат (LazyColumn), «Очистить историю» (диалог с чекбоксом «Также очистить память задачи»), поле ввода (команды /add_long_term, /add_task_memory, /help не уходят в модель), «Превысить контекст» (debug), «Отправить»; long-tap по сообщению — диалог «Извлечь факты и сохранить»; строка токенов; тосты; `imePadding()`, `adjustResize`.
...
- **Напоминалки (День 18):** запрос «напомни через N минут» → модель вызывает schedule_reminder → приложение регистрирует на MCP-сервере (SQLite) и ставит локальное уведомление (AlarmManager: setAlarmClock, при отказе — setAndAllowWhileIdle; ReminderReceiver). POST_NOTIFICATIONS запрашивается при старте (MainActivity). Запрос «какие напоминания?» → get_reminders(timezone) → список с сервера во времени пользователя (userTimezone из TimeZone.getDefault().id). После перезагрузки устройства напоминания не переустанавливаются.
...
- **Сборка индекса (JVM):** модуль `doc-index/` — нарезка корпуса репозитория (две стратегии), эмбеддинги через Ollama `nomic-embed-text`, SQLite `doc_index.sqlite`. Стратегия STRUCTURE: заголовки `#`–`######`, склейка длинных секций по абзацам, префикс названия раздела в `text` чанка, версия схемы `schema_version` в `index_meta` (после смены — пересборка `buildDocIndex`). Подробности — [doc-index/README.md](doc-index/README.md).
...
## RAG в агенте (День 22)

- **Поток:** при включённом `rag_enabled` (DataStore) и непустом `BuildConfig.OLLAMA_HOST` перед вызовом `SimpleAgent.process` ViewModel вызывает `RagContextBuilder.buildContext(userQuery)` на `Dispatchers.IO`: эмбеддинг текста запроса через `OllamaEmbeddingClient` (`/api/embeddings`, модель `nomic-embed-text`), `DocEmbeddingIndex.search(..., ChunkingStrategy.STRUCTURE, topK)`, сборка markdown через `RagMarkdownFormatter`. При ошибке Ollama/БД или пустом `OLLAMA_HOST` — тост, запрос к OpenAI не отправляется.
- **Уточнение поиска:** `RagRetrievalEnhancement` — перед merge кандидатов (STRUCTURE + FIXED_WINDOW) опционально дописывает к строке эмбеддинга (не к пользовательскому тексту в чате) и даёт лексический бонус при rerank для узких классов вопросов: индексация/RAG/эмбеддинги, стратегии контекста (`ContextStrategy`), поток данных экрана «Агент» (UI → API).
- **Промпт:** без tools — блок «Локальный индекс (фрагменты документации)» в user-промпте перед историей диалога; с tools (`processWithTools`) — тот же текст добавляется в system message перед блоком инвариантов.
- **Ручная проверка:** см. раздел «День 22: RAG» в [docs/AGENT_TEST_SCENARIO.md](docs/AGENT_TEST_SCENARIO.md).

...
- **Вызов MCP моделью по обычному промпту:** при заданном `MCP_CUSTOM_SERVER_URL` в запрос к OpenAI передаётся список tools (mock_echo, get_current_weather) в формате function calling. SimpleAgent.process() при наличии tools вызывает processWithTools(): собирает сообщения (system, история, новый user), в цикле (до 5 раундов) вызывает ChatRepository.sendOneCompletion(messages, tools); при ответе с tool_calls выполняет mock_echo → McpCustomClient.callMockEcho, get_current_weather → McpWeatherClient.getWeather, дописывает сообщения assistant + tool и повторяет запрос; финальный текст без tool_calls возвращается в чат. Статический список tools — data/AgentTools.kt; репозиторий — один round-trip (sendOneCompletion), без цикла. OpenAiDto: ChatCompletionRequest.tools, ChatMessage (роли user/assistant/system/tool, tool_calls, tool_call_id), MessageContent.toolCalls, десериализатор для tool_calls.

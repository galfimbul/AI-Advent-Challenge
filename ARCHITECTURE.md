...
- **UI:** чат — история диалога в виде прокручиваемого списка сообщений (LazyColumn), пузырьки «Вы» / «Агент»; кнопка «Очистить историю»; поле ввода; в одной строке кнопка «Превысить контекст» (только при BuildConfig.DEBUG) слева от кнопки «Отправить»; `imePadding()` и `adjustResize` в манифесте.
- **Тест превышения контекста (debug):** кнопка «Превысить контекст» вызывает `sendContextOverflowTest()` → `agent.process(dialog, "Тест: превышение контекста", forceContextOverflow = true)`. Агент дописывает к промпту большой объём текста, запрос превышает лимит контекста API; ошибка отображается в UI. Токены на экране приходят из `usage` в ответе API, приложение их не считает.

...
private const val RAG_PROMPT_INSTRUCTION =
  "\n\nПравила по фрагментам документации выше:\n" +
    "— Вопросы про этот репозиторий (где в проекте что задаётся, где лежит код, где ключ API OpenAI, какие файлы и настройки) отвечай в первую очередь по этим фрагментам.\n" +
    "— Не говори «в предоставленных фрагментах нет информации», если она явно есть в тексте; перескажи и укажи имя файла из строки «### …».\n" +
    "— Если нужного факта в фрагментах действительно нет — скажи об этом честно, без выдумывания.\n" +
    "— При ссылке на факты называй файлы так, как они указаны в заголовках фрагментов."
...
class SimpleAgent(
  ...
  suspend fun process(
    ...
    val prompt = buildString {
      ...
      if (!ragContextMarkdown.isNullOrBlank()) {
        append("Локальный индекс (фрагменты документации репозитория — приоритетный источник фактов о проекте):\n")
      ...
      append(trimmed)
      append("\n\nДай развёрнутый, но по существу ответ. Если выше есть фрагменты документации — используй их для фактов о проекте (пути, ключи, конфиги, файлы).")
    }
  ...
  private fun buildSystemMessageWithInvariants(
    ...
    if (!ragContextMarkdown.isNullOrBlank()) {
      append("\n\nЛокальный индекс (фрагменты документации репозитория — приоритетный источник фактов о проекте):\n")
      append(ragContextMarkdown)
      append(RAG_PROMPT_INSTRUCTION)
    }
